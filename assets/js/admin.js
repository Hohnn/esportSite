...
function addInputs(){
    ...
    col.innerHTML = `
    <div class="d-flex">
        <input type="text" class="form-control d-none me-1" name="playerName${playerCount}" data-player-name placeholder="Nom du joueur" required>
...
function listen(param){
    param.forEach(element => {
        ...
        if (user == 0){
            allPlayerName[dataSelect-1].classList.remove('d-none');
            memberLogo[dataSelect-1].src = `data:image/png;base64,${defaultLogoBase64}`;
        } else if (user > 0){
            allPlayerName[dataSelect-1].classList.add('d-none');
