...
if (isset($_POST['submitTeamUpdate'])) {
    ...
    if (!isValid($regexText, $name)) {
        $errorTeamName = 'Champ obligatoire';
        $count++;
    }
    if (!isValid($regexText, $tag)) {
    ...
    if ($count == 0 && empty($verifUpload)) {
        $Comp->updateTeam($_POST['teamId'], $name, $logoEncode, $_POST['country'], $tag, $_SESSION['id']);
        $team = $Comp->getTeam($_POST['teamId']);
        $playerCount = $_POST['playerCount'];
        $Comp->deletePlayerByTeam($_POST['teamId']);
